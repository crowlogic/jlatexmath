...
public class FontInfo
{
  ...
  {
    lig.put(new CharCouple(left,
                           right),
            new Character(ligChar));
  }
  ...
  {
    Object obj = kern.get(new CharCouple(left,
                                         right));
    if (obj == null)
      return 0;
    else
  ...
  {
    Object obj = lig.get(new CharCouple(left,
                                        right));
    if (obj == null)
      return null;
    else
      return new CharFont(((Character) obj).charValue(),
                          fontId);
  }
  ...
  {
    if (unicode == null)
      nextLarger[ch] = new CharFont(larger,
                                    fontLarger);
    else if (!unicode.containsKey(ch))
    {
      char s = (char) unicode.size();
      unicode.put(ch, s);
      nextLarger[s] = new CharFont(larger,
                                   fontLarger);
    }
    else
      nextLarger[unicode.get(ch)] = new CharFont(larger,
                                                 fontLarger);
  }
