...
public class TeXFormula
{
  ...
  static
  {
    ...
    try
    {
      DefaultTeXFont.registerAlphabet((AlphabetRegistration) Class.forName("org.scilab.forge.jlatexmath.cyrillic.CyrillicRegistration")
                                                                  .newInstance());
      DefaultTeXFont.registerAlphabet((AlphabetRegistration) Class.forName("org.scilab.forge.jlatexmath.greek.GreekRegistration")
                                                                  .newInstance());
    }
    catch (Exception e)
    {
  ...
  public static void addSymbolMappings(String file) throws ResourceParseException
  {
    ...
    {
      throw new ResourceParseException(file,
                                       e);
    }
  ...
  {
    TeXFormulaSettingsParser tfsp = new TeXFormulaSettingsParser(in,
                                                                 name);
  ...
  public static void registerExternalFont(Character.UnicodeBlock block, String sansserif, String serif)
  {
    ...
    externalFontMap.put(block,
                        new FontInfos(sansserif,
                                      serif));
  ...
  protected TeXFormula(TeXParser tp)
  {
    ...
    parser         = new TeXParser(tp.getIsPartial(),
                                   "",
                                   this,
                                   false);
  }
  ...
  protected TeXFormula(TeXParser tp, String s, boolean firstpass) throws ParseException
  {
    ...
    parser = new TeXParser(isPartial,
                           s,
                           this,
                           firstpass);
    if (isPartial)
  ...
  public static TeXFormula getAsText(String text, int alignment) throws ParseException
  {
    ...
    formula.add(new MatrixAtom(false,
                               atoms,
                               MatrixAtom.ARRAY,
                               alignment));
  ...
  public static TeXFormula getPartialTeXFormula(String formula)
  {
    ...
    TeXParser parser = new TeXParser(true,
                                     formula,
                                     f);
    try
    {
  ...
  public TeXFormula append(boolean isPartial, String s) throws ParseException
  {
    if (s != null && s.length() != 0)
    {
      TeXParser tp = new TeXParser(isPartial,
                                   s,
                                   this);
      tp.parse();
    }
  ...
  {
    new PredefinedTeXFormulaParser(xmlFile,
                                   "TeXFormula").parse(predefinedTeXFormulas);
  }

  public static void addPredefinedCommands(InputStream xmlFile) throws ResourceParseException
  {
    new PredefinedTeXFormulaParser(xmlFile,
                                   "Command").parse(MacroInfo.Commands);
  }
  ...
  {
    return add(new SpaceAtom(widthUnit,
                             width,
                             heightUnit,
                             height,
                             depthUnit,
  ...
  {
    return new TeXIconBuilder().setStyle(style)
                               .setSize(size)
                               .setType(type)
                               ...
                               float interline)
  {
    return new TeXIconBuilder().setStyle(style)
                               ...
                               .setType(type)
                               .setWidth(widthUnit, textwidth, align)
                               .setInterLineSpacing(interlineUnit, interline)
                               .build();
  }
   ...
   */
  public static Image
         createBufferedImage(String formula, int style, float size, Color fg, Color bg) throws ParseException
  {
    TeXFormula f    = new TeXFormula(formula);
    TeXIcon    icon = f.createTeXIcon(style, size);
    icon.setInsets(new Insets(2,
                              2,
                              2,
                              2));
    int           w     = icon.getIconWidth(), h = icon.getIconHeight();
  ...
  public TeXFormula setBackground(Color c)
  {
    if (c != null)
    {
      if (root instanceof ColorAtom)
        root = new ColorAtom(c,
                             null,
                             (ColorAtom) root);
      else
        root = new ColorAtom(root,
                             c,
                             null);
    }
    return this;
