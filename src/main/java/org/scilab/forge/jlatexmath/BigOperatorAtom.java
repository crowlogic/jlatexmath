 ...
 * with its limits.
 */
public class BigOperatorAtom extends
                             Atom
{
   ...
   * @param under  atom representing the under limit
   * @param over   atom representing the over limit
   * @param limits whether limits should be drawn over and under the base
   *               (&lt;-&gt; as scripts)
   */
  ...
  public Box createBox(TeXEnvironment env)
  {
                  ...
                  || (base.type_limits == TeXConstants.SCRIPT_NORMAL && style >= TeXConstants.STYLE_TEXT))
    {
      ...
      if (bbase != null)
      {
        bbase.add(new ScriptsAtom(base,
                                  under,
                                  over));
      ...
      return new ScriptsAtom(base,
                             under,
                             over).createBox(env);
    }
    else
    {
      if (base instanceof SymbolAtom && base.type == TeXConstants.TYPE_BIG_OPERATOR)
      { // single
        // bigop
        // symbol
      ...
      { // formula
        delta = 0;
        y     = new HorizontalBox(base == null ? new StrutBox(0,
                                                              0,
                                                              0,
                                                              0) : base.createBox(env));
      }
      ...
      float maxWidth = Math.max(Math.max(x == null ? 0 : x.getWidth(), y.getWidth()), z == null ? 0 : z.getWidth());
