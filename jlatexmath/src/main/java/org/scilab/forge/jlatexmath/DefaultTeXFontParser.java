...
public class DefaultTeXFontParser
{
  ...
  private static class ExtensionParser implements
                                       CharChildParser
  {
  ...
  private static class KernParser implements
                                  CharChildParser
  {
  ...
  private static class LigParser implements
                                 CharChildParser
  {
  ...
  private static class NextLargerParser implements
                                        CharChildParser
  {
  ...
  {
    this(DefaultTeXFontParser.class.getResourceAsStream(RESOURCE_NAME),
         RESOURCE_NAME);
  }
  ...
  public DefaultTeXFontParser(InputStream file, String name) throws ResourceParseException
  {
    ...
    { // JDOMException or IOException
      throw new XMLResourceParseException(name,
                                          e);
    }
  }
  ...
  public FontInfo[] parseFontDescriptions(FontInfo[] fi, InputStream file, String name) throws ResourceParseException
  {
    ...
    FontInfo info = new FontInfo(Font_ID.indexOf(fontId),
                                 base,
                                 path,
                                 fontName,
                                 unicode,
    ...
    for (int i = 0; i < list.getLength(); i++)
    {
      ...
      if (node.getNodeType() != Node.TEXT_NODE)
      {
        ...
        if (parser == null) // unknown element
          throw new XMLResourceParseException(RESOURCE_NAME + ": a <Char>-element has an unknown child element '"
                        + el.getTagName() + "'!");
        else
  ...
  public static Font createFont(InputStream fontIn, String name) throws ResourceParseException
  {
    try
    {
      ...
      if (shouldRegisterFonts)
      {
        try
        {
          Method registerFontMethod = graphicEnv.getClass().getMethod("registerFont", new Class[]
          { Font.class });
          if ((Boolean) registerFontMethod.invoke(graphicEnv, new Object[]
          { f }) == Boolean.FALSE)
          {
    ...
    {
      throw new XMLResourceParseException(RESOURCE_NAME + ": error reading font '" + name + "'. Error message: "
                    + e.getMessage());
    }
    finally
    {
      ...
      {
        throw new RuntimeException("Close threw exception",
                                   ioex);
      }
    }
  }
        ...
        try
        {
          if (base == null)
          {
            map = factory.newDocumentBuilder()
                         .parse(DefaultTeXFontParser.class.getResourceAsStream(include))
                         .getDocumentElement();
          }
          else
          {
            map = factory.newDocumentBuilder()
                         .parse(base.getClass().getResourceAsStream(include))
                         .getDocumentElement();
          }
        }
        catch (Exception e)
    ...
    { // element present
      ...
      for (int i = 0; i < list.getLength(); i++)
      {
        ...
        if (charFonts[index] == null) // range not defined
          throw new XMLResourceParseException(RESOURCE_NAME + ": the default text style mapping '" + textStyleName
                        + "' for the range '" + code + "' contains no mapping for that range!");
        else
  ...
  public static float
         getOptionalFloat(String attrName, Element element, float defaultValue) throws ResourceParseException
  {
    String attrValue = element.getAttribute(attrName);
    if (attrValue.equals("")) // attribute not present
      return defaultValue;
