 ...
 */
public class OverUnderDelimiter extends
                                Atom
{
  ...
  public OverUnderDelimiter(Atom base,
                            ...
                            int kernUnit,
                            float kern,
                            boolean over) throws InvalidUnitException
  {
    type        = TeXConstants.TYPE_INNER;
    this.base   = base;
    this.script = script;
    symbol      = s;
    this.kern   = new SpaceAtom(kernUnit,
                                0,
                                kern,
                                0);
    this.over   = over;
  ...
  public Box createBox(TeXEnvironment env)
  {
    ...
    del = new VerticalBox(del,
                          max,
                          TeXConstants.ALIGN_CENTER);
    if (scriptBox != null && max - scriptBox.getWidth() > TeXFormula.PREC)
    {
      scriptBox = new HorizontalBox(scriptBox,
                                    max,
                                    TeXConstants.ALIGN_CENTER);
    }

    return new OverUnderBox(b,
                            del,
                            scriptBox,
                            kern.createBox(env).getHeight(),
                            over);
  }
