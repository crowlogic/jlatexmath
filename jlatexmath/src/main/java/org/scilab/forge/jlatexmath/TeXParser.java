...
public class TeXParser
{
   ...
   * &amp; a new atom is added in the current line and when a \\ is met, a new
   * line is created.
   *
   * @param isPartial   if true certains exceptions are not thrown
   * @param parseString the string to be parsed
  ...
  private Atom getScripts(char f) throws ParseException
  {
    ...
    {
      at = new PhantomAtom(new CharAtom('M',
                                        "mathnormal"),
                           false,
                           true,
                           true);
    ...
    if (((c < '0' || c > '9') && (c < 'a' || c > 'z') && (c < 'A' || c > 'Z')))
    {
                    ...
                    && (TeXFormula.symbolFormulaMappings == null || TeXFormula.symbolFormulaMappings[c] == null))
      {
        ...
        if (fontInfos != null)
        {
          ...
          return new JavaFontRenderingAtom(parseString.substring(start, end + 1),
                                           fontInfos);
        }

        if (!isPartial)
        {
          throw new ParseException("Unknown character : '" + Character.toString(c) + "' (or " + ((int) c) + ")");
        }
        else
        {
  ...
  private Atom processEscape() throws ParseException
  {
    ...
    {
      return new ColorAtom(new RomanAtom(new TeXFormula("\\backslash " + command).root),
                           null,
                           Color.RED);
    }
  }
