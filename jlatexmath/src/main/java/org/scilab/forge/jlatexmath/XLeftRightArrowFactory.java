...
public class XLeftRightArrowFactory
{
  ...
  public static Box create(boolean left, TeXEnvironment env, float width)
  {
    ...
    Box   minus  = new SmashedAtom(MINUS,
                                   "").createBox(env);
    Box   kern   = new SpaceAtom(TeXConstants.UNIT_MU,
                                 -4f,
                                 0,
                                 0).createBox(env);
    ...
    hb.add(new SpaceAtom(TeXConstants.UNIT_MU,
                         -2f * sf,
                         0,
                         0).createBox(env));
    hb.add(new ScaleAtom(MINUS,
                         sf,
                         1).createBox(env));
  ...
  public static Box create(TeXEnvironment env, float width)
  {
    ...
    if (width < swidth)
    {
      ...
      hb.add(new StrutBox(-Math.min(swidth - width, left.getWidth()),
                          0,
                          0,
                          0));
      hb.add(right);
    ...
    hb.add(new ScaleBox(minus,
                        (width - swidth - w) / minus.getWidth(),
                        1));
